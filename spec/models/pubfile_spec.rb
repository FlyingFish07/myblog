...
describe Pubfile do
  ...
  it "should save ok with right name and url" do
     ...
     expect(pubfile1.pfile.url).to eq("/uploads/pubfiles/#{pubfile1.user.id}/sunshine.jpg")
  ...
  it "should save ok with right pinyin name and url" do
     ...
     expect(pubfile2.pfile.url).to eq("/uploads/pubfiles/#{pubfile2.user.id}/mei-li-de-sky.jpg")
  ...
  it "should not valid with same file name" do
     pubfile = FactoryGirl.create(:pubfile)
     expect(Pubfile.new(FactoryGirl.attributes_for(:pubfile).merge!({:user_id => pubfile.user_id}))).not_to be_valid
  end
  ...
  it "should update ok with right name and url" do
     ...
     expect(pubfile1.pfile.url).to eq("/uploads/pubfiles/#{pubfile1.user.id}/mei-li-de-sky.jpg")
