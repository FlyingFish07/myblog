module MyBootstrapPagination
  ...
  module BootstrapRenderer
    ...
    def ul_class
      ...
      options_class_ary = @options[:class].split
      if not @options[:page_links]
        options_class_ary.delete "pagination"
        options_class_ary << "pager" 
      end
      options_class_ary.compact.uniq.join(" ")
    end
  end
end